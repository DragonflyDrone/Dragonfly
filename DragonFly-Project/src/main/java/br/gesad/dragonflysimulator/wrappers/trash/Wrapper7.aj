...
package br.gesad.dragonfly.wrappers.trash;
...
import br.gesad.dragonflysimulator.model.entity.boat.BoatBusinessObject;
...
import br.gesad.dragonflysimulator.util.StopWatch;
import br.gesad.dragonflysimulator.view.CellView;
import br.gesad.dragonflysimulator.view.boat.BoatView;
import br.gesad.dragonflysimulator.view.drone.DroneView;
import br.gesad.dragonflysimulator.view.river.RiverView;

import java.br.gesad.dragonfly.util.*;
...
public aspect Wrapper7 {
    ...
    pointcut updateBatteryPerSecond(): call (* br.gesad.dragonfly.model.entity.drone.DroneBusinessObject.updateBatteryPerSecond(*));
    pointcut landing(): call (* br.gesad.dragonfly.model.entity.drone.DroneBusinessObject.landing(*));
    pointcut resetSettingsDrone(): call (void br.gesad.dragonfly.model.entity.drone.DroneBusinessObject.resetSettingsDrone(*));
    ...
    pointcut navigate() : call (void br.gesad.dragonfly.controller.BoatAutomaticController.navigate(*));

    //todo esses br.gesad.dragonfly.wrappers não foram testados para vários drones no mesmo ambiente
            ...
            public void task() {
                Platform.runLater(() -> {
                    ...
                    if (!dronesAreOnBoatInSet.contains(drone) && CellController.getInstance().calculeteDisplacementFrom(droneView.getCurrentCellView(), boatView.getCurrentCellView()) == 30) {
                        ...
                        imageViewDrone = new ImageView(new Image());
