...
IF "%IDE_BIN_DIR:~-8%" == "bin\win\" (
  echo.
  echo    You are trying to run DSL-Dragonfly from generic distribution
  echo Please first copy content of "%IDE_BIN_DIR%"
  echo into its parent directory ("%IDE_BIN_DIR:~0,-4%"^)
  echo and then run dsl-dragonfly.bat from there
  echo.
  pause
  exit
)
...
:: Try (in order): DSL-Dragonfly_JDK, dsl-dragonfly%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=
SET IDEA_VENDOR_NAME=null

IF EXIST "%DSL-Dragonfly_JDK%" SET JDK=%DSL-Dragonfly_JDK%
IF EXIST "%JDK%" GOTO check

SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\DSL-Dragonfly1.0\dsl-dragonfly%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\DSL-Dragonfly1.0\dsl-dragonfly%BITS%.exe.jdk
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start DSL-Dragonfly.
  ECHO No JDK found. Please validate either DSL-Dragonfly_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%DSL-Dragonfly_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%DSL-Dragonfly_PROPERTIES%"

:: explicit
SET VM_OPTIONS_FILE=%DSL-Dragonfly_VM_OPTIONS%
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\DSL-Dragonfly1.0\mps%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, standard installation
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\dsl-dragonfly%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, universal package
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\win\dsl-dragonfly%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET IDEA_PATHS_SELECTOR=DSL-Dragonfly1.0
SET PRODUCT="DSL-Dragonfly"
