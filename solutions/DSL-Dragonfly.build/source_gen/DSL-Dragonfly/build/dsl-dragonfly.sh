#!/bin/sh
# ---------------------------------------------------------------------
# DSL-Dragonfly startup script.
# ---------------------------------------------------------------------
# Generated by MPS

message()
{
  TITLE="Cannot start DSL-Dragonfly"
...
# Try (in order): $DSL-Dragonfly_JDK, .../dsl-dragonfly.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='null'
# shellcheck disable=SC2154
if [ -n "$DSL-Dragonfly_JDK" ] && [ -x "$DSL-Dragonfly_JDK/bin/java" ]; then
  JDK="$DSL-Dragonfly_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/DSL-Dragonfly1.0/dsl-dragonfly.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/DSL-Dragonfly1.0/dsl-dragonfly.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either DSL-Dragonfly_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$DSL-Dragonfly_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$DSL-Dragonfly_PROPERTIES"
fi
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/DSL-Dragonfly1.0/dsl-dragonfly$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/DSL-Dragonfly1.0/dsl-dragonfly$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/dsl-dragonfly$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/dsl-dragonfly$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/dsl-dragonfly$BITS.vmoptions"
fi
...
# shellcheck disable=SC2154
if [ -n "$DSL-Dragonfly_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$DSL-Dragonfly_CLASSPATH"
fi
...
IDEA_PATHS_SELECTOR=DSL-Dragonfly1.0
"$JAVA_BIN" \
  ${AGENT} \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_DSL-Dragonfly_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_DSL-Dragonfly.hprof" \
