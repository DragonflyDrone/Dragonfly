...
# Try (in order): DSL-Dragonfly_JDK, dsl-dragonfly.jdk, ../jre, JDK_HOME, JAVA_HOME, "java" in PATH.
# ---------------------------------------------------------------------
if [ -n "$DSL-Dragonfly_JDK" -a -x "$DSL-Dragonfly_JDK/bin/java" ]; then
  JDK="$DSL-Dragonfly_JDK"
elif [ -s "$HOME/.DSL-Dragonfly1.0/config/dsl-dragonfly.jdk" ]; then
  JDK=`"$CAT" $HOME/.DSL-Dragonfly1.0/config/dsl-dragonfly.jdk`
  if [ ! -d "$JDK" ]; then
    JDK="$IDE_HOME/$JDK"
  fi
...
elif [ -r "$HOME/.DSL-Dragonfly1.0/config/mps$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.DSL-Dragonfly1.0/config/mps$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=DSL-Dragonfly1.0"
  fi
fi
